...
export function ToursCustomizeContent() {
    ...
    const checkoutCopy = {
        ko: {
            ...
            failedError: "결제 페이지로 이동하는 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요.",
            processingLabel: "결제 페이지로 이동 중...",
        ...
        ja: {
            ...
            failedError: "決済ページへ移動中に問題が発生しました。しばらくしてから再度お試しください。",
            processingLabel: "決済ページへ移動中...",
        ...
        en: {
            ...
            failedError: "Something went wrong while opening the payment page.",
            processingLabel: "Opening payment page...",
    ...
    const paymentButtonLabel = checkoutSubmitting ? checkoutCopy.processingLabel : checkoutCopy.payLabel;
    const summaryRows = [
    ...
    const handleCustomPlanPayment = async () => {
        ...
        try {
            if (typeof window === "undefined") {
                throw new Error(checkoutCopy.failedError);
            }

            window.sessionStorage.setItem(
                CUSTOM_PLAN_PAYMENT_STORAGE_KEY,
                JSON.stringify({
                    title: customPlanTitle,
                    summary: buildCustomPlanSummary(),
                    totalAmount: estimatedPrice,
                    baseAmount: minPrice * guests,
                    optionAmount: optionTotal + activityTotal,
                    depositAmount: customPlanDeposit,
                    image: regionInfo.image,
            ...
            router.push(withLocaleQuery("/payment?custom=1", lang));
        } catch (caught) {
            ...
            {planCheckoutOpen && typeof document !== "undefined" ? createPortal(
                <div className="animate-fade-in-overlay fixed bottom-0 left-0 right-0 top-0 z-[80] flex h-dvh w-screen items-end justify-center bg-slate-950/55 backdrop-blur-md sm:items-center">
                    ...
                    <div
                        ...
                        className={`animate-slide-up-modal relative z-10 max-h-[92vh] w-full max-w-3xl overflow-y-auto overscroll-contain rounded-t-[30px] border shadow-2xl [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden sm:m-4 sm:rounded-[30px] ${isDark ? "border-white/10 bg-slate-900 text-slate-100" : "border-slate-200 bg-white text-slate-900"}`}
                    >
                        <div className="relative min-h-[220px] overflow-hidden rounded-t-[30px]">
                            <Image src={regionInfo.image} alt={regionInfo.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 768px" />
                            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/60 to-slate-950/10" />
                            <button
                                type="button"
                                onClick={() => setPlanCheckoutOpen(false)}
                                className="absolute right-5 top-5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/15 text-xl leading-none text-white backdrop-blur-md transition hover:bg-white/25"
                            >
                                x
                            </button>
                        ...
                        <div className="p-5 sm:p-7">
                            <div className={`rounded-[26px] border p-4 sm:p-5 ${isDark ? "border-white/10 bg-slate-950/80" : "border-slate-200 bg-slate-50"}`}>
                                ...
                                <div className={`mt-4 border-t pt-4 ${isDark ? "border-white/10" : "border-slate-200"}`}>
                                    <div className="grid gap-4 sm:grid-cols-3">
                                        {[
                                            ...
                                            <div key={label}>
                                                <div className={`text-xs font-bold ${isDark ? "text-slate-400" : "text-slate-500"}`}>{label}</div>
                                                <div className={`mt-1 text-sm font-bold leading-6 ${isDark ? "text-slate-200" : "text-slate-700"}`}>{value}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
